...
## Objective
Implement a complete JWT authentication service with refresh token rotation, blacklist management, and all security features as specified in the comprehensive specification.

## Description
Create a robust authentication service that handles JWT token generation, validation, refresh token rotation, blacklist management, and provides middleware for route protection. This includes all the security features outlined in the specification.

## Acceptance Criteria
- [ ] JWT service with dual token strategy (access + refresh)
- [ ] Refresh token rotation mechanism
- [ ] Token blacklist system implementation
- [ ] Password hashing and validation
- [ ] Authentication middleware for Gin
- [ ] Token cleanup background processes
- [ ] Complete error handling and security logging
...
func (a *AuthService) ValidateToken(tokenString string) (*Claims, error) {
    ...
    if claims.TokenType == models.TokenRefresh && claims.JTI != "" {
        blacklisted, err := a.isRefreshTokenBlacklisted(claims.JTI)
        if err != nil {
            return nil, err
        }
        if blacklisted {
...
func (a *AuthService) RotateRefreshToken(oldRefreshToken string) (string, string, error) {
    ...
    if claims.JTI != "" {
        err = a.BlacklistRefreshToken(claims.JTI, claims.UserID)
        if err != nil {
            return "", "", err
        }
    }
...
func (a *AuthService) BlacklistRefreshToken(jti, userID string) error {
    expiresAt := time.Now().Add(a.refreshTokenTTL)
    return a.queries.CreateRefreshTokenBlacklist(context.Background(), sqlc.CreateRefreshTokenBlacklistParams{
        Jti:       jti,
        UserID:    userID,
        ExpiresAt: expiresAt,
    })
...
func (a *AuthService) isRefreshTokenBlacklisted(jti string) (bool, error) {
    return a.queries.IsRefreshTokenBlacklisted(context.Background(), jti)
}

func (a *AuthService) CleanupExpiredTokens() error {
    return a.queries.CleanupExpiredBlacklistedTokens(context.Background())
}
...
3. Blacklist system prevents token reuse
