...
func migrate(
	...
	dsn := fmt.Sprintf(
		"postgres://%s:%s@%s/%s?application_name=anti-fraud-operation-manager&search_path=%s&connect_timeout=10",
...
func createSchema(
	...
	if _, err := conn.Exec(ctx, sql); err != nil {
		return errors.Join(
			conn.Close(ctx),
			fmt.Errorf("error creating schema: %w", err),
		)
	}
...
func toPoolConfig(config Config) (*pgxpool.Config, error) {
	dsn := fmt.Sprintf(
		"postgres://%s:%s@%s/%s?application_name=anti-fraud-operation-manager&search_path=%s",
		config.User, config.Password, config.host, config.Name, config.Schema,
	)
