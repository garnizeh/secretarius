...
func testCreateProject(db *database.DB) func(*testing.T) {
	return func(t *testing.T) {
		...
		err = db.Write(ctx, func(queries *store.Queries) error {
			...
			proj, err := queries.CreateProject(ctx, store.CreateProjectParams{
				Name:        "Test Project",
				Description: description,
				Color:       color,
				Status:      status,
...
func testCreateLogEntry(db *database.DB) func(*testing.T) {
	return func(t *testing.T) {
		...
		err = db.Write(ctx, func(queries *store.Queries) error {
			entry, err := queries.CreateLogEntry(ctx, store.CreateLogEntryParams{
				UserID:      userID,
...
func testTransactionRollback(db *database.DB) func(*testing.T) {
	return func(t *testing.T) {
		...
		err = db.Read(ctx, func(queries *store.Queries) error {
			_, err := queries.GetUserByEmail(ctx, "rollback-test@example.com")
			return err
		})
