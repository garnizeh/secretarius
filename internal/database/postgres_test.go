...
func TestDBOperations(t *testing.T) {
	...
	t.Run("ReadOperation", func(t *testing.T) {
		...
		err = db.Read(ctx, func(queries *store.Queries) error {
			user, err := queries.GetUserByID(ctx, createdUserID)
			if err != nil {
				return err
			}
			fetchedUser = user
...
func TestNoRows(t *testing.T) {
	...
	t.Run("NoRowsError", func(t *testing.T) {
		var err error
		readErr := db.Read(ctx, func(queries *store.Queries) error {
...
func TestComplexTransactions(t *testing.T) {
	...
	t.Run("MultipleOperationsInTransaction", func(t *testing.T) {
		...
		err := db.Write(ctx, func(queries *store.Queries) error {
			...
			// Create log entry
			logEntry, err := queries.CreateLogEntry(ctx, store.CreateLogEntryParams{
				UserID:      userID,
		...
		err = db.Read(ctx, func(queries *store.Queries) error {
			...
			_, err = queries.GetLogEntryByID(ctx, logEntryID)
			return err
		})
		require.NoError(t, err)
	})
...
func countUsers(t *testing.T, db *database.DB) int {
	...
	var count int64
	err := db.Read(ctx, func(queries *store.Queries) error {
		userCount, err := queries.GetUserCount(ctx)
		if err != nil {
			return err
		}
		count = userCount
